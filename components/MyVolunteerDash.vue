...
// Filtering volunteers based on status
...
// Fetch volunteers requested FROM ME
async function fetchVolunteersRequestedFromMe() {
  ...
  Promise.all(response.data.map(async (ven) => {
    ...
    volunteersRequestedFromMe.value = response.data;
    console.log(response.data)
  })
}

// Fetch volunteers I requested
async function fetchVolunteersIRequested() {
  ...
  Promise.all(response.data.map(async (ven) => {
    ven.volunteer.avatarSRC = await fetchImage(ven.volunteer.avatar_url, 'avatars')
    ven.volunteer.bannerSRC = await fetchImage(ven.volunteer.banner_url, 'images')
  })).then(() => {
    volunteersIRequested.value = response.data;
    console.log(response.data)
  })
...
async function updateRequestStatus(request, status) {
  const { error } = await supabase.from('VolunteerBooking').update({ status }).eq('id', request.id);
...
const newVolunteerConversationInfo = computed(() => getContactInfo(selectedBooking.value?.volunteer));
      ...
      <main class="flex-1 p-8">
        <Tabs default-value="requested-from-me" class="w-full">
          ...
          <TabsContent value="requested-from-me" class="space-y-6">
            ...
            <div class="flex">
              <div class="w-1/3">
                <div v-for="item in filterRequests(volunteersRequestedFromMe, showRequested, showOffered, showApproved, showRejected, showCancelled)"
          ...
          <TabsContent value="requested" class="space-y-6">
            <p class="text-gray-800 p-2 font-semibold">
              View the volunteers that you have requested or been offered:
            </p>
            <div class="flex">
              <div class="w-1/3">
                <div v-for="item in filterRequests(volunteersIRequested, showRequested, showOffered, showApproved, showRejected, showCancelled)"
                  :class="{ 'flex flex-col w-full border border-black-300 space-y-0 p-2 rounded-lg shadow-lg transition-transform transform hover:shadow-2xl dark:bg-[#1A1A1A]border-2 border-orange-500 bg-orange-100': selectedBooking.id == item.id,
                    'flex flex-col w-full border border-black-300 space-y-0 bg-white p-2 rounded-lg shadow-lg transition-transform transform hover:shadow-2xl dark:bg-[#1A1A1A]': selectedBooking.id != item.id
                   }"
                  @click="selectBooking(item)">
                  <div class="flex">
                    <div class="flex flex-1 flex-col items-center">
                      <Avatar class="w-16 h-16">
                        <AvatarImage :src="item.volunteer.avatarSRC" alt="User's Name" />
                        <AvatarFallback>UN</AvatarFallback>
                      </Avatar>
                      <h3 class="font-semibold text-center text-md mt-2">{{ item.volunteer.full_name }}</h3>
                    </div>
                    <div class="flex grow flex-col items-center">
                      <h6 class="font-semibold text-large text-gray-500">{{ item?.event?.title }}</h6>
                    </div>
              ...
              <div class="w-2/3">
                <VolunteerBookingCard v-if="Object.keys(selectedBooking).length !== 0" :profile="selectedBooking?.volunteer" :event-name="selectedBooking?.event?.title" :note="selectedBooking?.note">
                  <template #action-buttons>
                    <MessagesButton :label="'Contact'" :isIcon="false"
                          :newConversationInfo="newVolunteerConversationInfo"></MessagesButton>
