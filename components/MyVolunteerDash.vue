...
// State
const volunteersRequestedFromMe = ref([]);
const volunteersIRequested = ref([]);
...
const stats = computed(() => [
  {
    ...
    value: volunteersRequestedFromMe.value.length,
    icon: Book,
    trend: '+12%',
    color: 'text-blue-500',
    bgColor: 'bg-blue-50'
  },
  {
    ...
    value: volunteersRequestedFromMe.value.filter(e => e.status === 'requested').length,
    icon: Clock,
    trend: '+5%',
  ...
  {
    label: 'Approved',
    value: volunteersRequestedFromMe.value.filter(e => e.status === 'approved').length,
    icon: CheckCircle,
    trend: '+18%',
  ...
  },
  {
    ...
    value: volunteersRequestedFromMe.value.filter(e => e.status === 'rejected' || v.status === 'cancelled').length,
    icon: XCircle,
    trend: '-2%',
    color: 'text-red-500',
    bgColor: 'bg-red-50'
...
function filterRequests(requests, requested, offered, approved, rejected, cancelled) {
  return requests.filter(req => {
    const matchesSearch = searchQuery.value === '' || 
      ...
      req.volunteer?.full_name.toLowerCase().includes(searchQuery.value.toLowerCase());
...
async function fetchVolunteersRequestedFromMe() {
  const { data, error } = await supabase
    .from('VolunteerBooking')
    .select(`
      *,
      event:AllEvents!inner(*,
        event_owner:profiles!inner(*)
      ),
      volunteer:profiles!inner(*,
        volunteer:Volunteers!inner(*)
      )
    `)
    .eq('volunteer.id', user.value.id);
  ...
  Promise.all(
    data.map(async (volunteer) => {
      volunteer.event.event_owner.avatarSRC = await fetchImage(
        volunteer.event.event_owner.avatar_url,
        'avatars'
      );
      return volunteer;
    })
  ).then((volunteers) => {
    volunteersRequestedFromMe.value = volunteers;
  });
}

async function fetchVolunteersIRequested() {
  const { data, error } = await supabase
    .from('VolunteerBooking')
    .select(`
      *,
  ...
  Promise.all(
    data.map(async (volunteer) => {
      volunteer.volunteer.avatarSRC = await fetchImage(
        volunteer.volunteer.avatar_url,
        'avatars'
      );
      volunteer.volunteer.bannerSRC = await fetchImage(
        volunteer.volunteer.banner_url,
        'images'
      );
      return volunteer;
    })
  ).then((volunteers) => {
    volunteersIRequested.value = volunteers;
  });
}
...
async function updateRequestStatus(request, status) {
  ...
  await Promise.all([
    fetchVolunteersRequestedFromMe(),
    fetchVolunteersIRequested()
  ]);
}
...
const newVolunteerConversationInfo = computed(() => 
  getContactInfo(selectedBooking.value?.volunteer)
);
...
<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="bg-white rounded-2xl shadow-sm mb-6">
      <div class="p-6"><div class="flex flex-row items-center justify-between"><h1 class="text-2xl font-bold text-gray-900">Volunteer Management</h1> <NuxtLink to="profile" class="m-6 items-center">
      <Button  class="bg-orange-500 text-white dark:bg-orange-600 dark:text-white hover:bg-gray-100 hover:text-orange-500">Manage my Profile</Button>
    </NuxtLink>
  </div>
        <p class="text-gray-600 mt-1">Manage your volunteer bookings and requests</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-1 space-y-4">
        <div 
          v-for="booking in activeTab === 'requested-from-me' 
            ? filterRequests(volunteersRequestedFromMe, ...Object.values(filters))
            : filterRequests(volunteersIRequested, ...Object.values(filters))"
          :key="booking.id"
      ...
      <div class="lg:col-span-2">
        <div v-if="selectedBooking.id" class="bg-white rounded-xl shadow-sm border border-gray-100">
          <div class="p-6">
            <div class="flex items-center justify-between mb-6">
              <h2 class="text-xl font-bold text-gray-900">Volunteer Booking Details</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <!-- Volunteer Details -->
              <div class="space-y-4">
                <div class="flex"><h3 class="font-medium text-gray-900">Volunteer Information</h3>
                  <NuxtLink :to="{ name: 'VolunteerCardPage', query: { id: selectedBooking.volunteer.id } }">
                ...
                <div class="space-y-2">
                  <p class="text-sm">
                    ...
                    {{ activeTab === 'requested-from-me'
                      ? selectedBooking.volunteer.full_name
                      : selectedBooking.volunteer.full_name }}
                  </p>
                  <p class="text-sm">
                    <span class="font-medium text-gray-700">Volunteerise:</span>
                    {{ selectedBooking.volunteer.volunteer.headline }}
                  </p>
                  <p class="text-sm">
                    <span class="font-medium text-gray-700">Categories:</span>
                    {{ selectedBooking.volunteer.volunteer.categories?.join(', ') }}
                  </p>
                </div>
              </div>
            ...
            <div class="mt-6 flex items-center justify-end space-x-3">
              ...
              :start-conversation-user-id="activeTab === 'requested' ? selectedBooking.volunteer.id : selectedBooking.event.event_owner.id"></MessagesButton>
        ...
        >
          ...
          <h3 class="text-lg font-medium text-gray-900">No Volunteer Selected</h3>
          <p class="text-gray-600 mt-2">Select an volunteer booking from the list to view its details</p>
        </div>
      </div>
    </div>
  </div>
