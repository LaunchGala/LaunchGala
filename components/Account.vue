...
import { User, Edit2, Mail, MapPin, Check, Linkedin, Twitter, Youtube, Facebook, PlusCircle, TwitchIcon, GhostIcon, InstagramIcon } from 'lucide-vue-next';
import {
  Accordion,
  AccordionItem,
  AccordionTrigger,
  ...
  try {
    ...
    if (userProfile.value.influencer.categories.length > 0) {
      const influencerUpdates = {
        ...
        tiktok: userProfile.value.influencer.tiktok,
        instagram: userProfile.value.influencer.instagram,
        snapchat: userProfile.value.influencer.snapchat,
        twitch: userProfile.value.influencer.twitch,
        discord: userProfile.value.influencer.discord,
      };
    ...
    // Step 6: Upsert `positions` information
    if (userProfile.value.positions.length > 0) {
      // Prepare data for insertion
      const positionsPayload = userProfile.value.positions
        .filter((position) => position.company?.id !== undefined) // Filter out positions without a valid company ID
        .map((position) => ({
          profile_id: user.value.id, // Profile ID
          company_id: position.company.id, // Company ID
          job_title: position.job_title, // Job title
          is_owner: position.company.createdBy === user.value.id, // Check ownership
        }));
      ...
      try {
        ...
        const { data, error } = await supabase
          ...
          .upsert(positionsPayload, { onConflict: ['profile_id', 'company_id'] }); // Specify the conflict keys
...
function addNewPosition(){
  ...
  userProfile.value.positions.push({
    job_title: newPosition.value.job_title,
    company: unref(newPosition.value.company)
  });
  ...
  <div class="mx-auto max-w-4xl py-8 px-6 bg-white dark:bg-black rounded-lg shadow-md space-y-6">
    <div class="border-b dark:border-gray-700 pb-4">
      ...
      <div class="flex items-center space-x-4">
        <div class="flex-grow">
          <ProfileAvatar v-model:path="userProfile.avatar_path" v-model:bannerPath="userProfile.banner_path"@upload="updateProfile" :hide-upload="editDisabled"/>
      ...
      <div>
        ...
        <div class="mt-4 space-y-4">
          <div>
            ...
            <Popover>
              <PopoverTrigger :disabled="editDisabled" class="mb-4 w-full rounded-md text-sm border border-gray-300 dark:border-orange-600 bg-white dark:bg-gray-800 py-2 px-3 flex justify-between items-center">
                ...
                <ScrollArea class="h-80 w-full p-1">
                  <ul class="menu">
                    <li v-for="industry in filteredIndustries" :key="industry" class="flex items-center gap-2">
                      <input type="checkbox" :checked="userProfile.industries.includes(industry)" @click="toggleIndustrySelection(industry)" />
                      <span>{{ industry }}</span>
                    </li>
                  </ul>
                </ScrollArea>
          ...
          <div class="w-full">
            <label class="block text-sm font-semibold text-gray-600 dark:text-gray-400" for="positions">Positions</label>
      ...
      <div>
        ...
        <div class="mt-4 space-y-4">
          <div>
            ...
            <Input id="email" :disabled="editDisabled" v-model="userProfile.email" class="mt-1 w-full rounded-md bg-gray-100 dark:bg-gray-800 border-transparent focus:border-gray-400 dark:focus:border-gray-700" />
          </div>
          <div>
            <label class="block text-sm font-semibold text-gray-600 dark:text-gray-400" for="phone">Phone</label>
            <Input id="phone" :disabled="editDisabled" v-model="userProfile.phone" class="mt-1 w-full rounded-md bg-gray-100 dark:bg-gray-800 border-transparent focus:border-gray-400 dark:focus:border-gray-700" />
          </div>
          <div>
        <DropdownMenu>
          <DropdownMenuTrigger :disabled="editDisabled" class="mb-4 w-full rounded-md border border-gray-300 dark:border-orange-600 bg-white dark:bg-gray-800 py-2 px-3 flex justify-between items-center">
          ...
          <DropdownMenuContent class="rounded-md border border-orange-300 dark:border-orange-600 bg-white dark:bg-gray-800 shadow-lg py-1">
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem @click="userProfile.location = ''" class="hover:bg-orange-200 dark:hover:bg-orange-700">
          ...
          <div>
            ...
            <Input id="website" :disabled="editDisabled" v-model="userProfile.website" class="mt-1 w-full rounded-md bg-gray-100 dark:bg-gray-800 border-transparent focus:border-gray-400 dark:focus:border-gray-700" />
          </div>
        </div>
      </div>
    </div>
    <div class="mt-4 flex space-x-2">
      <div class="flex items-center flex-1 bg-gray-100 dark:bg-gray-800 rounded-md">
        ...
        <Input type="text" :disabled="editDisabled" placeholder="LinkedIn URL" v-model="userProfile.linkedin" class="flex-1 border-0 focus:ring-2 focus:ring-blue-500" />
      </div>
      <div class="flex items-center flex-1 bg-gray-100 dark:bg-gray-800 rounded-md">
        <Twitter class="w-6 h-6 ml-3 text-black-500" />
        <Input type="text" :disabled="editDisabled" placeholder="Twitter URL" v-model="userProfile.twitter" class="flex-1 border-0 focus:ring-2 focus:ring-blue-500" />
      </div>
      <div class="flex items-center flex-1 bg-gray-100 dark:bg-gray-800 rounded-md">
        <Youtube class="w-6 h-6 ml-3 text-red-500" />
        <Input type="text" :disabled="editDisabled" placeholder="YouTube URL" v-model="userProfile.youtube" class="flex-1 border-0 focus:ring-2 focus:ring-pink-500" />
      </div>
      <div class="flex items-center flex-1 bg-gray-100 dark:bg-gray-800 rounded-md">
        <Facebook class="w-6 h-6 ml-3 text-blue-600" />
        <Input type="text" :disabled="editDisabled" placeholder="Facebook URL" v-model="userProfile.facebook" class="flex-1 border-0 focus:ring-2 focus:ring-blue-500" />
      </div>
    </div>
    <div>
      <div class="flex items-center space-x-2">
      ...
      <Textarea :disabled="editDisabled" v-model="userProfile.about" class="mt-1 w-full rounded-md bg-gray-100 dark:bg-gray-800 border-transparent focus:border-gray-400 dark:focus:border-gray-700" rows="4"></Textarea>
    </div>
    <!-- Accordion Section -->
        ...
        <AccordionContent>
          <div class="mt-4 space-y-4">
            <div class="grid grid-cols-2">
              <div>
                <label class="block text-sm font-semibold text-gray-600 dark:text-gray-400" for="expertWillTravel">List as Expert</label>
                <Switch :disabled="editDisabled" :default-checked="userProfile.is_expert" @update:checked="setIsExpert" id="expertWillTravel" />
              </div>
            <div>
              <label class="block text-sm font-semibold text-gray-600 dark:text-gray-400" for="expertHeadline">Headline</label>
              <Input id="expertHeadline" :disabled="editDisabled" v-model="userProfile.expert.headline" class="mt-1 w-full rounded-md bg-gray-100 dark:bg-gray-800 border-transparent focus:border-gray-400 dark:focus:border-gray-700" />
            </div>
              <div>
                <label class="block text-sm font-semibold text-gray-600 dark:text-gray-400" for="expertWillTravel">Willing to Travel</label>
                <Switch :disabled="editDisabled" :default-checked="userProfile.expert.will_travel" @update:checked="setExpertWillTravel" id="expertWillTravel" />
              </div>
              <div>
                <label class="block text-sm font-semibold text-gray-600 dark:text-gray-400" for="expert_category">Expert Type</label>
                <Popover>
                  <PopoverTrigger :disabled="editDisabled" class="flex mt-1 w-full rounded-md bg-gray-100 dark:bg-gray-800 border-transparent focus:border-gray-400 dark:focus:border-gray-700 p-2 text-sm text-left">
                    <ChevronDown class="w-5 h-5 mr-2 text-orange-600 dark:text-orange-400" />
                    <span>{{ userProfile.expert.categories.length ? userProfile.expert.categories.join(', ') : 'Select' }}</span>
                  </PopoverTrigger>
                  <PopoverContent class="border rounded-md border-orange-300 dark:border-orange-600 bg-white dark:bg-gray-800 shadow-lg">
                    <div class="p-2">
                    ...
                    <ScrollArea class="h-80 w-full p-1">
                      <ul class="menu">
                        <li v-for="expert_category in filteredExpertCategories" :key="expert_category" class="flex items-center gap-2">
                          <Checkbox :checked="userProfile.expert.categories.includes(expert_category)" @click="toggleExpertCategorySelection(expert_category)" />
      ...
      <AccordionItem value="volunteer">
        ...
        <AccordionContent>
          <div class="mt-4 space-y-4">
            <div class="grid grid-cols-2">
              <div>
                <label class="block text-sm font-semibold text-gray-600 dark:text-gray-400" for="isVolunteer">List as Volunteer</label>
                <Switch :disabled="editDisabled" :default-checked="userProfile.is_volunteer" @update:checked="setIsVolunteer" id="isVolunteer" />
              </div>
            <div>
              <label class="block text-sm font-semibold text-gray-600 dark:text-gray-400" for="expertVolunteer">Headline</label>
              <Input id="expertVolunteer" :disabled="editDisabled" v-model="userProfile.volunteer.headline" class="mt-1 w-full rounded-md bg-gray-100 dark:bg-gray-800 border-transparent focus:border-gray-400 dark:focus:border-gray-700" />
            </div>
            <div>
              <label class="block text-sm font-semibold text-gray-600 dark:text-gray-400" for="volunteerWillTravel">Willing to Travel</label>
              <Switch :disabled="editDisabled" :default-checked="userProfile.volunteer.will_travel" @update:checked="setVolunteerWillTravel" id="displayPrice" />
            </div>
            <div>
            ...
            <Popover>
              <PopoverTrigger :disabled="editDisabled" class="flex mt-1 w-full rounded-md bg-gray-100 dark:bg-gray-800 border-transparent focus:border-gray-400 dark:focus:border-gray-700 p-2 text-sm text-left">
                <ChevronDown class="w-5 h-5 mr-2 text-orange-600 dark:text-orange-400" />
                <span>{{ userProfile.volunteer.categories.length ? userProfile.volunteer.categories.join(', ') : 'Select one' }}</span>
              </PopoverTrigger>
              <PopoverContent class=" border rounded-md border-orange-300 dark:border-orange-600 bg-white dark:bg-gray-800 shadow-lg ">
                <div class="p-2">
                ...
                <ScrollArea class="h-80 w-full p-1">
                  <ul class="menu">
                    <li v-for="volunteer_category in filteredVolunteerCategories" :key="volunteer_category" class="flex items-center gap-2">
                      <input type="checkbox" :checked="userProfile.volunteer.categories.includes(volunteer_category)" @click="toggleVolunteerCategorySelection(volunteer_category)" />
                      {{ volunteer_category }}
                    </li>
                  </ul>
                </ScrollArea>
      ...
      <AccordionItem value="influencer">
        ...
        <AccordionContent>
          <div class="mt-4 space-y-4">
            <div class="grid grid-cols-2">
              <div>
                <label class="block text-sm font-semibold text-gray-600 dark:text-gray-400" for="isInfluencer">List as Influencer</label>
                <Switch :disabled="editDisabled" :default-checked="userProfile.is_influencer" @update:checked="setIsInfluencer" id="isInfluencer" />
              </div>
            <div>
              <label class="block text-sm font-semibold text-gray-600 dark:text-gray-400" for="influencerHeadline">Headline</label>
              <Input id="influencerHeadline" :disabled="editDisabled" v-model="userProfile.influencer.headline" class="mt-1 w-full rounded-md bg-gray-100 dark:bg-gray-800 border-transparent focus:border-gray-400 dark:focus:border-gray-700" />
            </div>
            <div>
              ...
              <Switch :disabled="editDisabled" :default-checked="userProfile.influencer.will_travel" @update:checked="setInfluencerWillTravel" id="displayPrice" /></div>
            <div>
            <label class="block text-sm font-semibold text-gray-600 dark:text-gray-400" for="influencer">Influencer Type</label>
            <Popover>
              <PopoverTrigger :disabled="editDisabled" class="flex mt-1 w-full rounded-md bg-gray-100 dark:bg-gray-800 border-transparent focus:border-gray-400 dark:focus:border-gray-700 p-2 text-sm text-left">
                <ChevronDown class="w-5 h-5 mr-2 text-orange-600 dark:text-orange-400" />
                <span>{{ userProfile.influencer.categories.length ? userProfile.influencer.categories.join(', ') : 'Select one' }}</span>
              </PopoverTrigger>
              <PopoverContent class=" border rounded-md border-orange-300 dark:border-orange-600 bg-white dark:bg-gray-800 shadow-lg ">
                <div class="p-2">
                ...
                <ScrollArea class="h-80 w-full p-1">
                  <ul class="menu">
                    <li v-for="influencer_category in filteredInfluencerCategories" :key="influencer_category" class="flex items-center gap-2">
                      <input type="checkbox" :checked="userProfile.influencer.categories.includes(influencer_category)" @click="toggleInfluencerCategorySelection(influencer_category)" />
                      {{ influencer_category }}
                    </li>
                  </ul>
                </ScrollArea>
          ...
          <div class="mt-4 flex space-x-2">
            <div class="flex items-center flex-1 bg-gray-100 dark:bg-gray-800 rounded-md">
              ...
              <Input type="text" :disabled="editDisabled" placeholder="TikTok URL" v-model="userProfile.influencer.tiktok" class="flex-1 border-0 focus:ring-2 focus:ring-blue-500" />
            </div>
            <div class="flex items-center flex-1 bg-gray-100 dark:bg-gray-800 rounded-md">
              <InstagramIcon class="w-6 h-6 ml-3 text-black-500" />
              <Input type="text" :disabled="editDisabled" placeholder="Instagram URL" v-model="userProfile.influencer.instagram" class="flex-1 border-0 focus:ring-2 focus:ring-blue-500" />
            </div>
            <div class="flex items-center flex-1 bg-gray-100 dark:bg-gray-800 rounded-md">
              <CIcon :icon="cibSnapchat" class="w-6 h-6 ml-3 text-blue-500" />
              <Input type="text" :disabled="editDisabled" placeholder="Snapchat URL" v-model="userProfile.influencer.snapchat" class="flex-1 border-0 focus:ring-2 focus:ring-pink-500" />
            </div>
            <div class="flex items-center flex-1 bg-gray-100 dark:bg-gray-800 rounded-md">
              <TwitchIcon class="w-6 h-6 ml-3 text-blue-600" />
              <Input type="text" :disabled="editDisabled" placeholder="Twitch URL" v-model="userProfile.influencer.twitch" class="flex-1 border-0 focus:ring-2 focus:ring-blue-500" />
            </div>
            <div class="flex items-center flex-1 bg-gray-100 dark:bg-gray-800 rounded-md">
              <DiscordLogoIcon class="w-6 h-6 ml-3 text-blue-600" />
              <Input type="text" :disabled="editDisabled" placeholder="Discord URL" v-model="userProfile.influencer.discord" class="flex-1 border-0 focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
      ...
      <AccordionItem value="talent">
        ...
        <AccordionContent>
          <div class="mt-4 space-y-4">
            <div class="grid grid-cols-2">
              <div>
                <label class="block text-sm font-semibold text-gray-600 dark:text-gray-400" for="isTalent">List as Talent</label>
                <Switch :disabled="editDisabled" :default-checked="userProfile.is_talent" @update:checked="setIsTalen" id="isTalent" />
              </div>
            <div>
              <label class="block text-sm font-semibold text-gray-600 dark:text-gray-400" for="talentHeadline">Headline</label>
              <Input id="talentHeadline" :disabled="editDisabled" v-model="userProfile.talent.headline" class="mt-1 w-full rounded-md bg-gray-100 dark:bg-gray-800 border-transparent focus:border-gray-400 dark:focus:border-gray-700" />
            </div>
            <div>
            ...
            <Popover>
              <PopoverTrigger :disabled="editDisabled" class="flex mt-1 w-full rounded-md bg-gray-100 dark:bg-gray-800 border-transparent focus:border-gray-400 dark:focus:border-gray-700 p-2 text-sm text-left">
                <ChevronDown class="w-5 h-5 mr-2 text-orange-600 dark:text-orange-400" />
                <span>{{ userProfile.talent.employment_type.length ? userProfile.talent.employment_type.join(', ') : 'Select one' }}</span>
              </PopoverTrigger>
              <PopoverContent class=" border rounded-md border-orange-300 dark:border-orange-600 bg-white dark:bg-gray-800 shadow-lg ">
                <div class="p-2">
                ...
                <ScrollArea class="h-80 w-full p-1">
                  <ul class="menu">
                    <li v-for="emp_type in filteredEmploymentTypes" :key="emp_type" class="flex items-center gap-2">
                      <input type="checkbox" :checked="userProfile.talent.employment_type.includes(emp_type)" @click="toggleEmploymentTypeSelection(emp_type)" />
                      {{ emp_type }}
                    </li>
                  </ul>
                </ScrollArea>
