...
// State
const expertsRequestedFromMe = ref([]);
const expertsIRequested = ref([]);
...
const stats = computed(() => [
  {
    ...
    value: expertsRequestedFromMe.value.length,
    icon: Book,
    trend: '+12%',
    color: 'text-blue-500',
    bgColor: 'bg-blue-50'
  },
  {
    ...
    value: expertsRequestedFromMe.value.filter(e => e.status === 'requested').length,
    icon: Clock,
    trend: '+5%',
  ...
  {
    label: 'Approved',
    value: expertsRequestedFromMe.value.filter(e => e.status === 'approved').length,
    icon: CheckCircle,
    trend: '+18%',
  ...
  },
  {
    ...
    value: expertsRequestedFromMe.value.filter(e => e.status === 'rejected' || v.status === 'cancelled').length,
    icon: XCircle,
    trend: '-2%',
    color: 'text-red-500',
    bgColor: 'bg-red-50'
...
function filterRequests(requests, requested, offered, approved, rejected, cancelled) {
  return requests.filter(req => {
    const matchesSearch = searchQuery.value === '' || 
      ...
      req.expert?.full_name.toLowerCase().includes(searchQuery.value.toLowerCase());
...
async function fetchExpertsRequestedFromMe() {
  const { data, error } = await supabase
    ...
    .select(`
      *,
      ...
      expert:profiles!inner(*,
        expert:Experts!inner(*)
      )
    `)
    .eq('expert.id', user.value.id);
  ...
  Promise.all(
    data.map(async (expert) => {
      expert.event.event_owner.avatarSRC = await fetchImage(
        expert.event.event_owner.avatar_url,
        'avatars'
      );
      return expert;
    })
  ).then((experts) => {
    expertsRequestedFromMe.value = experts;
  });
}

async function fetchExpertsIRequested() {
  const { data, error } = await supabase
    .from('ExpertBooking')
    .select(`
      *,
  ...
  Promise.all(
    data.map(async (expert) => {
      expert.expert.avatarSRC = await fetchImage(
        expert.expert.avatar_url,
        'avatars'
      );
      expert.expert.bannerSRC = await fetchImage(
        expert.expert.banner_url,
        'images'
      );
      return expert;
    })
  ).then((experts) => {
    expertsIRequested.value = experts;
  });
}
...
async function updateRequestStatus(request, status) {
  ...
  await Promise.all([
    fetchExpertsRequestedFromMe(),
    fetchExpertsIRequested()
  ]);
}
...
const newExpertConversationInfo = computed(() => 
  getContactInfo(selectedBooking.value?.expert)
);
...
<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="bg-white rounded-2xl shadow-sm mb-6">
      <div class="p-6"><div class="flex flex-row items-center justify-between"><h1 class="text-2xl font-bold text-gray-900">Expert Management</h1> <NuxtLink to="profile" class="m-6 items-center">
      <Button  class="bg-orange-500 text-white dark:bg-orange-600 dark:text-white hover:bg-gray-100 hover:text-orange-500">Manage my Profile</Button>
        ...
        <p class="text-gray-600 mt-1">Manage your expert bookings and requests</p>
          ...
          <div class="flex items-center space-x-4">
            <div class="relative">
              <input
                v-model="searchQuery"
                type="text"
                placeholder="Search experts or events..."
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-1 space-y-4">
        <div 
          v-for="booking in activeTab === 'requested-from-me' 
            ? filterRequests(expertsRequestedFromMe, ...Object.values(filters))
            : filterRequests(expertsIRequested, ...Object.values(filters))"
          :key="booking.id"
      ...
      <div class="lg:col-span-2">
        <div v-if="selectedBooking.id" class="bg-white rounded-xl shadow-sm border border-gray-100">
          <div class="p-6">
            <div class="flex items-center justify-between mb-6">
              <h2 class="text-xl font-bold text-gray-900">Expert Booking Details</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <!-- Expert Details -->
              <div class="space-y-4">
                <div class="flex"><h3 class="font-medium text-gray-900">Expert Information</h3>
                  <NuxtLink :to="{ name: 'ExpertCardPage', query: { id: selectedBooking.expert.id } }">
                ...
                <div class="space-y-2">
                  <p class="text-sm">
                    ...
                    {{ activeTab === 'requested-from-me'
                      ? selectedBooking.expert.full_name
                      : selectedBooking.expert.full_name }}
                  </p>
                  <p class="text-sm">
                    <span class="font-medium text-gray-700">Expertise:</span>
                    {{ selectedBooking.expert.expert.headline }}
                  </p>
                  <p class="text-sm">
                    <span class="font-medium text-gray-700">Categories:</span>
                    {{ selectedBooking.expert.expert.categories?.join(', ') }}
                  </p>
                </div>
              </div>
            ...
            <div class="mt-6 flex items-center justify-end space-x-3">
              ...
              :start-conversation-user-id="activeTab === 'requested' ? selectedBooking.expert.id : selectedBooking.event.event_owner.id"></MessagesButton>
        ...
        >
          ...
          <h3 class="text-lg font-medium text-gray-900">No Expert Selected</h3>
          <p class="text-gray-600 mt-2">Select an expert booking from the list to view its details</p>
        </div>
      </div>
    </div>
  </div>
