...
const message = ref("Request Volunteer");
...
async function requestVolunteer() {
  ...
  const { error } = await supabase.from('VolunteerBooking').insert([booking]);
...
<template>
  <div class="bg-white dark:bg-black p-6 rounded-md shadow-lg w-full max-h-fit">
    <h3 class="font-semibold text-lg mb-4">Request a Volunteer</h3>
