...
const message = ref("Request Influencer");
...
async function checkDisable() {
  try {
    ...
    if (count > 0) {
      ...
      message.value = "Request InfluencerBooking";
    }
  } catch (error) {
...
async function requestVolunteer() {
  ...
  const { error } = await supabase.from('InfluencerBooking').insert([booking]);

  if (error) {
    console.error('Error inserting data:', error);
  } else {
    toast.success('Volunteer Request Sent', { timeout: 5000 });
    note.value = '';
    checkDisable();
  }
}
...
<template>
  <div class="bg-white dark:bg-black p-6 rounded-md shadow-lg w-full max-h-fit">
    <h3 class="font-semibold text-lg mb-4">Request a Influencer</h3>
