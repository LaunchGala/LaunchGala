...
const setIsTBD = (value: boolean) => {
  props.event.is_tbd = value
}
</script>
      ...
      <div class="container mx-auto px-4 lg:px-8 dark:bg-orange-200 ">
        <Card>
          ...
          <div class="flex flex-col items-center">
            <span class="text-sm text-gray-500 dark:text-gray-300 text-center block mt-2 align-middle flex items-center">
              <Switch :default-checked="props.event.is_tbd" @update:checked="setIsTBD" id="displayPrice" class="mr-2" />Event is TBD (To Be Determined)</span>
            </div>
          <div
            class="dark:bg-black bg-orange-100 text-gray-800 dark:text-white p-8 space-y-4 rounded-lg shadow-lg max-w-lg mx-auto mt-8" v-if="!props.event.is_tbd">
            <div class="grid gap-4 sm:grid-cols-2" >
              <div>
                ...
                <Popover>
                  <PopoverTrigger as-child>
                    <Button variant="outline" class="w-full justify-between bg-white">
                      <span>{{ props.event.event_start_date ? format(props.event.event_start_date, "PPP") : "Choose"
                        }}</span>
                    </Button>
                  </PopoverTrigger>
                ...
                <div class="mt-2 flex items-center rounded border border-gray-300 bg-white">
                  <Input type="time" v-model="props.event.event_start_time" placeholder="HH:MM"
                    class="w-full border-none bg-transparent pl-3 pr-2 text-gray-800 dark:text-white focus:ring-0" />
                </div>
              </div>
              <div>
                ...
                <Popover>
                  <PopoverTrigger as-child>
                    <Button variant="outline" class="w-full justify-between bg-white">
                      <span>{{ props.event.event_end_date ? format(props.event.event_end_date, "PPP") : "Choose"
                        }}</span>
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent class="w-auto p-0">
                ...
                <div class="mt-2 flex items-center rounded border border-gray-300 bg-white">
                  <Input type="time" v-model="props.event.event_end_time" placeholder="HH:MM"
                    class="w-full border-none bg-transparent pl-3 pr-2 text-gray-800 dark:text-white focus:ring-0" />
                </div>
              </div>
            </div>
        ...
        <SelectContent>
          ...
          <div class="flex justify-between items-center p-4">
            <Button @click="$emit('previousStep')" as-child variant="default"
              class="bg-white text-orange-500 border-orange-500 hover:bg-orange-100 font-bold mr-2">
              <div class="flex items-center">
                <ArrowLeft class="w-4 h-4 mr-2" />
                Back
              </div>
            </Button>
            <Progress :model-value="37" class=" text-white " />
            <Button @click="$emit('nextStep')" as-child variant="default"
              class="bg-white text-orange-500 border-orange-500 hover:bg-orange-100 font-bold ml-2">
              <div class="flex items-center">
                Next
